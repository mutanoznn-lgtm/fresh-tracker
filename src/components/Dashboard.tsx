  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-5xl">
        ...
        {/* Search & Copy */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="mb-4 flex items-center gap-2">
          ...
          <motion.button
            ...
            className={`flex-shrink-0 rounded-lg border p-2.5 transition-colors ${showAllProducts ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary hover:text-primary"}`}
            title={showAllProducts ? "Ocultar produtos de outros usuários" : "Ver produtos de outros usuários"}
          >
            <Users className="h-4 w-4" />
          </motion.button>
          {products.length > 0 && (
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleCopy}
              className="flex-shrink-0 rounded-lg border border-border p-2.5 text-muted-foreground transition-colors hover:border-secondary hover:text-secondary"
              title="Copiar Lista">
              {copied ? <CheckCircle className="h-4 w-4 text-neon-green" /> : <Copy className="h-4 w-4" />}
            </motion.button>
          )}
        </motion.div>
